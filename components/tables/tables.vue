  ...
  <div>
    <div v-if="searchable && searchPlace === 'top'" class="search-con search-con-top">
      <Select v-model="searchKey" class="search-col">
        <Option v-for="item in purecolumns" :key="`search-col-${item.key}`" :value="item.key">{{ item.title }}</Option>
      </Select>
...
<script>
import { getType } from '@keepfe/plugin-functions'
...
export default {
  ...
  computed: {
    purecolumns () {
      return this.columns.filter((item) => {
        return item.key && item.key !== 'handle'
      })
    }
  },
  watch: {
  ...
  methods: {
    ...
    handleSearch () {
      // 这里加个判断，有可能出现Number类型
      this.insideTableData = this.value.filter((item) => {
        if(getType(item[this.searchKey]) === 'number') {
          return parseInt(this.searchValue) === item[this.searchKey]
        } else if (getType(item[this.searchKey]) === 'string') {
